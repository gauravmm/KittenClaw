...
- `uv run python -m kittenclaw --once "your message"` *is* safe to run: it
  ...
  polling. It defaults to debug `chat_id` 0, so successive `--once`
  calls continue the same `conversations/0-*.jsonl` thread - delete that
  file to start fresh. Pass `--chat N` to override the id and keep separate,
  independently resumable threads. Use it to reproduce and debug what the
  bot does yourself. It does still call the model (consumes API quota).
